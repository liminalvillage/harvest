<script lang="ts">
    ...
    // 1. Not loading AND
    // 2. No personal holons exist
    $: {
        const personalHolonsCount = myHolons.filter(h => h.isPersonal).length;
        showSplashScreen = !isLoading && personalHolonsCount === 0;
    ...
    async function updateHolonDetails() {
        ...
        try {
            const updates = await Promise.all(
                myHolons.map(async (holon) => {
                    try {
                        ...
                        return holon; // Return original holon if update fails
                    }
                })
            );
            ...
            if (updates && updates.length > 0) {
                ...
                console.warn('No valid updates received, keeping original data');
            }
        } catch (err) {
    ...
    <div class="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <div class="max-w-2xl w-full bg-gray-800/90 rounded-lg shadow-xl p-8 text-gray-100">
            ...
            <div class="space-y-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-start space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-gray-300">Copy the holon ID and add it to the dashboard using the button below, or use the direct link from <code class="bg-gray-600 px-2 py-1 rounded">/dashboard</code></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Add First Holon Button -->
                <div class="text-center pt-6">
                    <button
                        on:click={() => showAddDialog = true}
                        class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-lg text-lg font-semibold transition-colors flex items-center justify-center space-x-3 mx-auto"
                    >
